...
export const gridIsolatedConfigs = pgTable("grid_isolated_configs", {
  ...
  takerFallbackAuditRequired:    boolean("taker_fallback_audit_required").notNull().default(true),
  // Wallet / Cartera
  gridWalletMode:                text("grid_wallet_mode").notNull().default("automatic"),
  gridWalletInitialUsd:          decimal("grid_wallet_initial_usd", { precision: 12, scale: 2 }).notNull().default("1000.00"),
  gridWalletMaxUsd:              decimal("grid_wallet_max_usd", { precision: 12, scale: 2 }).notNull().default("5000.00"),
  gridWalletUseProfits:          boolean("grid_wallet_use_profits").notNull().default(true),
  gridWalletCompoundProfits:     boolean("grid_wallet_compound_profits").notNull().default(true),
  gridMaxCapitalPerCycleUsd:     decimal("grid_max_capital_per_cycle_usd", { precision: 12, scale: 2 }).notNull().default("600.00"),
  gridMaxCapitalPerCyclePct:     decimal("grid_max_capital_per_cycle_pct", { precision: 6, scale: 2 }).notNull().default("60.00"),
  gridReservePct:                decimal("grid_reserve_pct", { precision: 6, scale: 2 }).notNull().default("20.00"),
  ...
  circuitBreakerSourceCycleId: text("circuit_breaker_source_cycle_id"),
  circuitBreakerSeverity:  text("circuit_breaker_severity"),
  circuitBreakerReviewAfter: timestamp("circuit_breaker_review_after", { withTimezone: true }),
  circuitBreakerResolvedAt: timestamp("circuit_breaker_resolved_at", { withTimezone: true }),
  circuitBreakerResolvedBy: text("circuit_breaker_resolved_by"),
  circuitBreakerResolutionReason: text("circuit_breaker_resolution_reason"),
  createdAt:               timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const gridIsolatedLevels = pgTable("grid_isolated_levels", {
  ...
  buyMakerPendingAt:       timestamp("buy_maker_pending_at", { withTimezone: true }),
  buyMakerPendingTickId:   integer("buy_maker_pending_tick_id"),
  buyMakerRequestedPrice:  decimal("buy_maker_requested_price", { precision: 18, scale: 8 }),
  createdAt:               timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const gridIsolatedCycles = pgTable("grid_isolated_cycles", {
  ...
  targetSellLevelId:       text("target_sell_level_id").references(() => gridIsolatedLevels.id),
  targetRungLevelId:       text("target_rung_level_id").references(() => gridIsolatedLevels.id),
  buyPrice:                decimal("buy_price", { precision: 18, scale: 8 }),
  sellPrice:               decimal("sell_price", { precision: 18, scale: 8 }),
  targetSellPrice:         decimal("target_sell_price", { precision: 18, scale: 8 }),
  targetSellQuantity:      decimal("target_sell_quantity", { precision: 18, scale: 8 }),
  exitPolicyVersion:       text("exit_policy_version"),
  targetKind:              text("target_kind"),
  targetCalculationJson:   jsonb("target_calculation_json"),
  riskStateJson:           jsonb("risk_state_json"),
  makerExitStateJson:      jsonb("maker_exit_state_json"),
  quantity:                decimal("quantity", { precision: 18, scale: 8 }).notNull(),
