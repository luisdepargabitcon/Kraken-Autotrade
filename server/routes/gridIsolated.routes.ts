...
function buildChatGPTSummary(mode: string, checks: any, status: any, blockingReasons: string[], levels: any[], cycles: any[], events: any[]): string {
  const lines: string[] = [];
  ...
  if (mode === "OFF") {
    lines.push("  1. Activar SHADOW para evaluación simulada.");
  }
  if (!checks.reconciliationPassed) {
    lines.push("  2. Ejecutar reconciliación manual.");
  }
  if (!checks.capitalReserved) {
    lines.push("  3. Reservar capital para Grid Isolated.");
  }
  if (!checks.postOnlySupported) {
    lines.push("  4. Confirmar soporte post-only en RevolutXService.");
  }
  return lines.join("\n");
}
...
export function registerGridIsolatedRoutes(app: Express): void {
  ...
  app.get("/api/grid-isolated/monitor/audit", async (_req: Request, res: Response) => {
    try {
      ...
      const chatgptSummary = buildChatGPTSummary(mode, checks, status, blockingReasons, levels, cycles, events);
  ...
  app.get("/api/grid-isolated/export/chatgpt", async (_req: Request, res: Response) => {
    try {
      ...
      const summary = buildChatGPTSummary(mode, checks, status, blockingReasons, levels, cycles, events);
