...
export class DatabaseStorage implements IStorage {
  ...
  async runSchemaMigration(): Promise<{ success: boolean; columnsAdded: string[]; error?: string }> {
    ...
    try {
      ...
      // === 018: Market Metrics tables + bot_config column ===
      try {
        await db.execute(sql`
        ...
        `);
        columnsAdded.push('market_metrics_snapshots (table)');
      ...
      try {
        ...
        `);
        columnsAdded.push('market_metrics_evaluations (table)');
      } catch (e) { /* already exists */ }

      try {
        await db.execute(sql`
          ALTER TABLE bot_config ADD COLUMN IF NOT EXISTS market_metrics_config JSONB
        `);
        columnsAdded.push('bot_config.market_metrics_config');
      } catch (e) { /* already exists */ }
      ...
      // === 029: IDCA VWAP Anchor persistent memory ===
      try {
        await db.execute(sql`
